...
class AttendanceTrackerApp:
    ...
    def setup_ui(self):
        ...
        self.summary_frame = ctk.CTkFrame(self.root, corner_radius=20, border_width=3, border_color=("#3b82f6", "#1d4ed8"))
        self.summary_frame.grid(row=1, column=0, sticky="ew", padx=20, pady=(5, 10))
        self.summary_frame.grid_columnconfigure((0, 1, 2), weight=1)
        
        self.lbl_global_percent = ctk.CTkLabel(self.summary_frame, text="Total: 0%", font=ctk.CTkFont(size=24, weight="bold"))
        self.lbl_global_percent.grid(row=0, column=0, pady=20, padx=20)
        self.lbl_safe_count = ctk.CTkLabel(self.summary_frame, text="SAFE: 0", font=ctk.CTkFont(size=20, weight="bold"), text_color="#10b981")
        self.lbl_safe_count.grid(row=0, column=1, pady=20, padx=20)
        self.lbl_short_count = ctk.CTkLabel(self.summary_frame, text="SHORT: 0", font=ctk.CTkFont(size=20, weight="bold"), text_color="#f43f5e")
        self.lbl_short_count.grid(row=0, column=2, pady=20, padx=20)
        
        self.scrollable_frame = ctk.CTkScrollableFrame(self.root, corner_radius=15, fg_color="transparent")
        self.scrollable_frame.grid(row=2, column=0, sticky="nsew", padx=20, pady=(10, 20))
        self.scrollable_frame.grid_columnconfigure(0, weight=1)
        
        self.empty_lbl = ctk.CTkLabel(self.scrollable_frame, text="No subjects created yet.\nClick 'Add Subject' to begin tracking!", font=ctk.CTkFont(size=18), text_color="gray")
    ...
    def refresh_cards(self):
        ...
        if not self.data:
            self.empty_lbl.grid(row=0, column=0, pady=100)
        else:
            self.empty_lbl.grid_forget()
            
        for s_name in list(self.subject_widgets.keys()):
            if s_name not in self.data:
                self.subject_widgets[s_name]["card"].destroy()
                del self.subject_widgets[s_name]
        ...
        for idx, (subject_name, details) in enumerate(self.data.items()):
            ...
            p = (a / t * 100) if t > 0 else 0.0
            if p >= target:
                safe_count += 1
            else:
                short_count += 1
                
            if subject_name in self.subject_widgets:
                self.update_subject_card(idx, subject_name, details)
            else:
                self.create_subject_card(idx, subject_name, details)
                
        global_p = (total_attended / total_classes * 100) if total_classes > 0 else 0.0
        self.lbl_global_percent.configure(text=f"Total: {global_p:.1f}%")
        self.lbl_safe_count.configure(text=f"SAFE: {safe_count}")
        self.lbl_short_count.configure(text=f"SHORT: {short_count}")

    def update_subject_card(self, row_idx, subject_name, details):
        widgets = self.subject_widgets[subject_name]
        widgets["card"].grid(row=row_idx, column=0, sticky="ew", padx=15, pady=12)
        ...
        widgets["stats_lbl"].configure(text=f"Attended: {attended}/{total}  •  Target: {target}%")
        widgets["action_lbl"].configure(text=main_action_text, text_color=primary_color)
        widgets["prog_bar"].set(percentage / 100.0)
        widgets["prog_bar"].configure(progress_color=(primary_color if percentage > 0 else "gray"))
        widgets["percent_lbl"].configure(text=f"Current Status: {percentage:.1f}% ({status_text})")
    ...
    def create_subject_card(self, row_idx, subject_name, details):
        ...
        self.subject_widgets[subject_name] = {
            "card": card,
            "title_lbl": title_lbl,
            "stats_lbl": stats_lbl,
            "action_lbl": action_lbl,
            "prog_bar": prog_bar,
            "percent_lbl": percent_lbl
        }
